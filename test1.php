...
<div class="customade">
    <div class="image-container" ondrop="drop(event)" ondragover="allowDrop(event)">
        ...
        <img id="backgroundImage" src="images/product/infront.jpg" alt="Background Image" style="width: 300px; height: 300px;">
            ...
            <input type="file" id="overlayImageUpload" accept="image/*" onchange="displayOverlayImage(event)">
            <br>
    ...
    <div class="image-container1" ondrop="drops(event)" ondragover="allowDrops(event)">
        <div id="content">
        <img id="backgroundImage" src="images/product/back.png" alt="Background Image" style="width: 300px; height: 300px;">
            <img id="overlayImages" src="" alt="Overlay Image" class="overlay-image1" draggable="true"
                ondragstart="drags(event)">
            <br>
        </div>
            <label for="overlayImageUpload"><b>Upload Image:</b></label>
            <input type="file" id="overlayImageUpload" accept="image/*" onchange="displayOverlayImages(event)">
            <br>
        <label for="resize">Resize:</label>
        <input type="range" id="resize" min="20" max="300" value="100" oninput="resizeImages()">
        <br>
        <label for="rotate">Rotate:</label>
        <input type="range" id="rotate" min="0" max="360" value="0" oninput="rotateImages()">
        <br>
        <button onclick="captureScreenshots()" class="btn btn-primary">Capture Screenshot</button>
        <a href="customize.php"><button class="btn btn-danger">Back</button></a>
    </div>
</div>
...
<script>
    ...
    function drag(event) {
        event.dataTransfer.setData("texts", event.target.id);
    }

    function drags(events) {
    ...
    function drops(event) {
        event.preventDefault();
        const overlay = document.getElementById("overlayImages");
        overlay.style.left = event.clientX - overlay.clientWidth / 2 + 'px';
        overlay.style.top = event.clientY - overlay.clientHeight / 2 + 'px';
    }
    ...
    function displayOverlayImages(event) {
        ...
        reader.onload = function (e) {
            const overlayImage = document.getElementById("overlayImages");
            overlayImage.src = e.target.result;
        };
